...
DEFAULT_DATA_LOCAL_DIR = "/share/datasets/PhysicalAI-Autonomous-Vehicles/"
...
def normalize_record(row_dict: dict[str, Any]) -> dict[str, Any]:
    return {key: normalize_record_value(value) for key, value in row_dict.items()}


def first_non_empty(row_dict: dict[str, Any], keys: tuple[str, ...]) -> Any:
    for key in keys:
...
def resolve_ground_truth(row_dict: dict[str, Any]) -> Any:
    value = first_non_empty(row_dict, ("ground_truth",))
    if value is not None:
        return value
    reward_model = row_dict.get("reward_model")
    if isinstance(reward_model, dict):
        value = first_non_empty(reward_model, ("ground_truth",))
        if value is not None:
            return value
    value = first_non_empty(row_dict, ("answer", "coc", "cot", "gt_cot", "reasoning", "target"))
    if value is not None:
        return value
    extra_info = row_dict.get("extra_info")
    if isinstance(extra_info, dict):
        value = first_non_empty(extra_info, ("ground_truth", "coc", "cot", "gt_cot", "reasoning"))
        if value is not None:
            return value
    return ""
...
class AlpamayoDemoDataset3(Dataset):
    def __init__(
        self,
        ...
        self.data_local_dir = config.get("data_local_dir", DEFAULT_DATA_LOCAL_DIR)
        
        self._read_files()
        self.avdi = PhysicalAIAVDatasetLocalInterface(local_dir=self.data_local_dir)
    ...
    def _read_one_file(self, data_file: str) -> Any:
        ...
        if data_file.endswith(".parquet"):
            try:
                ...
                raise ImportError("pandas is required to read Alpamayo parquet metadata files.") from exc

            dataframe = pd.read_parquet(data_file)
            required_columns = {self.clip_id_key, "t0_us"}
            missing_columns = required_columns - set(dataframe.columns)
            if missing_columns:
                raise ValueError(f"Parquet file {data_file} is missing required columns: {sorted(missing_columns)}")
            return [normalize_record(record) for record in dataframe.to_dict("records")]
    ...
    def _build_messages_from_clip(self, row_dict: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        ...
        loaded_sample = load_physical_aiavdataset(clip_id, t0_us=int(row_dict.get("t0_us", self.default_t0_us)), avdi=self.avdi, maybe_stream=False)
        ...
        clip_meta = {
            ...
            "ego_history_xyz": loaded_sample.get("ego_history_xyz"),
            "ego_history_rot": loaded_sample.get("ego_history_rot"),
            "ego_future_xyz": loaded_sample.get("ego_future_xyz"),
            "ego_future_rot": loaded_sample.get("ego_future_rot"),
            "gt_cot": loaded_sample.get("cot") or loaded_sample.get("coc"),
        }
    ...
    def __getitem__(self, item: int) -> dict[str, Any]:
        ...
        if not row_dict.get("data_source"):
        ...
        row_index = row_dict.get("idx", item)
        extra_info.setdefault("index", int(row_index) if row_index is not None else item)
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained("/workspace/Alpamayo-R1-10B-vlm/")
    dataset = AlpamayoDemoDataset3(
